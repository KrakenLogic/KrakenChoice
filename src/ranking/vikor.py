...
class Vikor(MADMBase):
    ...
    def solve(self, weights, gama):
        ...
        #Calculating S
        S = ((best_option - self.data) / (best_option - worst_option) * weights).sum(axis = 1)
        S_positive = min(S)
        S_negative = max(S)
    
        #Calculating R
        R = np.max(((best_option - self.data) / (best_option - worst_option) * weights), axis = 1)
        R_positive = min(R)
        R_negative = max(R)
    
        #Calculating Q
        Q = (gama) * ((S - S_positive) / (S_negative - S_positive)) + (1 - gama) * ((R - R_positive) / (R_negative - R_positive))
